...
allShuffle()
{
    ...
    printf("# Generated by experiment %s.\n"
            "# Note: latency numbers are displayed in us.\n"
            "#%12s%12s%8s%12s%12s%12s%12s%12s%20s\n",
            workload.empty() ? "default" : workload.c_str(), "nodes", "msg (B)",
            "ops", "min", "avg", "p50", "p90", "p99", "Throughput (Gbps)");
    printf("#----------------------------------------------------"
            "----------------------------------------\n");
...
#if 0
        double shuffleMBs = double(messageSize) * (machineCount - 1) /
                (1024 * 1024);
