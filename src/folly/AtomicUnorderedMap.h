...
inline constexpr size_t nextPowTwo(size_t v) {
    v--;
    v |= v >> 1;
    v |= v >> 2;
    v |= v >> 4;
...
struct AtomicUnorderedInsertMap {
  ...
  shardRange(IndexType numShards, IndexType shardId) const {
    IndexType maxSlotsPerShard = (numSlots_ + numShards - 1) / numShards;
    IndexType eslot = shardId * maxSlotsPerShard;
    IndexType bslot = std::min(eslot + maxSlotsPerShard - 1,
            IndexType(numSlots_) - 1);
    if (eslot > 0) {
        --eslot;
    }
    ...
    while (eslot > 0 && slots_[eslot].state() != LINKED) {
      --eslot;
    }
    return std::make_pair(ConstIterator(*this, bslot),
        ConstIterator(*this, eslot));
  }
 ...
 private:
  ...
  IndexType allocationAttempt(IndexType start, IndexType tries) const {
    if (__builtin_expect(!!(tries < 8 && start + tries < numSlots_), 1)) {
//    if (LIKELY(tries < 8 && start + tries < numSlots_)) {
      return IndexType(start + tries);
    } else {
      IndexType rv;
      if (sizeof(IndexType) <= 4) {
        rv = IndexType(RAMCloud::Util::wyhash32() % numSlots_);
//        rv = IndexType(folly::Random::rand32(numSlots_));
      } else {
        rv = IndexType(RAMCloud::Util::wyhash64() % numSlots_);
