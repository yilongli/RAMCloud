...
double
Driver::txNewOrder(uint32_t W_ID, bool* outcome)
{
    ///////////////////////////
    // Input Data Generation
    ///////////////////////////
    uint32_t D_ID = random(1, 10);
    uint32_t C_ID = NURand(1023, 1, 3000);
    uint8_t O_OL_CNT = random8(5, 15);
    ...
    for (int i = 0; i < O_OL_CNT; ++i) {
        ...
        if (rand() % 100 == 0) {
            OL_SUPPLY_W_ID[i] = random(1, context.numWarehouse);
        } else {
...
double
Driver::txPayment(uint32_t W_ID, bool *outcome)
{
    ///////////////////////////
    // Input Data Generation
    ///////////////////////////
    uint32_t D_ID = random(1, 10);
    uint32_t C_ID = 0, C_W_ID, C_D_ID;
    bool byLastName = false;

    cNameKey nameKey;
    char* lastName = nameKey.lastName;
    ...
    if (rand() % 100 < 85) {
        ...
        C_D_ID = random(1, 10);
        C_W_ID = random(1, context.numWarehouse); //Exclude local warehouse?
    }
    if (rand() % 100 < 60) {
        byLastName = true;
    ...
    double H_AMOUNT = (double)random(100, 500000) / 100.0;
    ...
    if (byLastName) {
        ...
        cids.reserve(numCustomer);
        for (int i = 0; i < numCustomer; ++i) {
            cids[i] = *(buf_cid.getOffset<uint32_t>(buf_idx));
            buf_idx += sizeof32(cids[i]);
        }
        C_ID = cids[(int)((numCustomer - 1) / 2)];
    }
    assert(C_ID);
...
double
Driver::txOrderStatus(uint32_t W_ID, bool *outcome)
{
    ///////////////////////////
    // Input Data Generation
    ///////////////////////////
...
double
Driver::txDelivery(uint32_t W_ID, bool* outcome)
{
    ///////////////////////////
    // Input Data Generation
    ///////////////////////////
    uint8_t O_CARRIER_ID = random8(1, 10);
    ...
    for (uint32_t D_ID = 1; D_ID <= 10; ++D_ID) {
        // Avoid contention for District. Okay to have stale data.
        // TODO: use MULTI_READ;
        District d(D_ID, W_ID);
        readRow(&t, &d);
        ...
        Order o(O_ID, D_ID, W_ID);
        readRow(&t, &o);
        o.data.O_CARRIER_ID = O_CARRIER_ID;
        writeRow(&t, &o);
...
Driver::txStockLevel(uint32_t W_ID, uint32_t D_ID, bool* outcome)
{
    ...
    // Input Data Generation
    ///////////////////////////
    uint16_t threshold = random16(10, 20);
