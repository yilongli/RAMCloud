...
class Warehouse : public Row {
  public:
     ...
     */
    explicit Warehouse(uint32_t W_ID) : type(1), W_ID(W_ID), data() {}

    virtual uint64_t tid() { return tableId[W_ID]; }
    virtual const void* pKey() { return &type; }
    virtual uint16_t pKeyLength() { return 1 + sizeof(W_ID); }
...
class District : public Row {
  public:
    ...
    explicit District(uint32_t D_ID, uint32_t D_W_ID)
        : type(2), D_ID(D_ID), D_W_ID(D_W_ID), data() {}

    virtual uint64_t tid() { return tableId[D_W_ID]; }
    virtual const void* pKey() { return &type; }
    virtual uint16_t pKeyLength() { return 1 + sizeof(D_ID) + sizeof(D_W_ID); }
    virtual const void* value() { return &data; }
    virtual uint32_t valueLength() { return sizeof(data); }
    virtual void parseBuffer(Buffer& buf) { data = *buf.getStart<Data>(); }
};
...
class Customer : public Row {
  public:
    ...
    uint32_t feature; //cLastOid table. Included in key for cLastOid.
    ...
    explicit Customer(uint32_t C_ID, uint32_t C_D_ID, uint32_t C_W_ID)
        : type(3), C_ID(C_ID), C_D_ID(C_D_ID), C_W_ID(C_W_ID), feature(1), data() {}

    virtual uint64_t tid() { return tableId[C_W_ID]; }
    virtual const void* pKey() { return &type; }
    virtual uint16_t pKeyLength() {
       return 1 + sizeof(C_D_ID) + sizeof(C_W_ID) + sizeof(C_ID);
    }
    virtual uint16_t lastOidKeyLength() {
       return 1 + sizeof(C_D_ID) + sizeof(C_W_ID) + sizeof(C_ID) + 1;
    }
    virtual const void* value() { return &data; }
    virtual uint32_t valueLength() { return sizeof(data); }
    virtual void parseBuffer(Buffer& buf) { data = *buf.getStart<Data>(); }
...
class History : public Row {
  public:
    ...
    explicit History(uint32_t H_C_ID, uint32_t H_C_D_ID, uint32_t H_C_W_ID)
        : type(4), H_C_ID(H_C_ID), H_C_D_ID(H_C_D_ID), H_C_W_ID(H_C_W_ID), data() {}

    virtual uint64_t tid() { return tableId[H_C_W_ID]; }
    virtual const void* pKey() { return &type; }
    virtual uint16_t pKeyLength() {
        return 1 + sizeof(H_C_D_ID) + sizeof(H_C_W_ID) + sizeof(H_C_ID);
    }
    virtual const void* value() { return &data; }
    virtual uint32_t valueLength() { return sizeof(data); }
    virtual void parseBuffer(Buffer& buf) { data = *buf.getStart<Data>(); }
...
class NewOrder : public Row {
  public:
    ...
    explicit NewOrder(uint32_t NO_O_ID, uint32_t NO_D_ID, uint32_t NO_W_ID)
        : type(5), NO_O_ID(NO_O_ID), NO_D_ID(NO_D_ID), NO_W_ID(NO_W_ID), data() {}

    virtual uint64_t tid() { return tableId[NO_W_ID]; }
    virtual const void* pKey() { return &type; }
    virtual uint16_t pKeyLength() {
        return 1 + sizeof(NO_D_ID) + sizeof(NO_W_ID) + sizeof(NO_O_ID);
    }
    virtual const void* value() { return &data; }
    virtual uint32_t valueLength() { return sizeof(data); }
    virtual void parseBuffer(Buffer& buf) { data = *buf.getStart<Data>(); }
...
class Order : public Row {
  public:
    ...
    explicit Order(uint32_t O_ID, uint32_t O_D_ID, uint32_t O_W_ID)
        : type(6), O_ID(O_ID), O_D_ID(O_D_ID), O_W_ID(O_W_ID), data() {}

    virtual uint64_t tid() { return tableId[O_W_ID]; }
    virtual const void* pKey() { return &type; }
    virtual uint16_t pKeyLength() {
        return 1 + sizeof(O_D_ID) + sizeof(O_W_ID) + sizeof(O_ID);
    }
    virtual const void* value() { return &data; }
    virtual uint32_t valueLength() { return sizeof(data); }
    virtual void parseBuffer(Buffer& buf) { data = *buf.getStart<Data>(); }
  ...
  public:
    ...
    explicit OrderLine(uint32_t OL_O_ID, uint32_t OL_D_ID, uint32_t OL_W_ID,
        ...
        : type(7), OL_O_ID(OL_O_ID), OL_D_ID(OL_D_ID), OL_W_ID(OL_W_ID),
          OL_NUMBER(OL_NUMBER), data() {}

    virtual uint64_t tid() { return tableId[OL_W_ID]; }
    virtual const void* pKey() { return &type; }
    virtual uint16_t pKeyLength() { return 1 + sizeof(OL_D_ID) + sizeof(OL_W_ID)
                                        + sizeof(OL_O_ID) + sizeof(OL_NUMBER); }
    virtual const void* value() { return &data; }
    virtual uint32_t valueLength() { return sizeof(data); }
    virtual void parseBuffer(Buffer& buf) { data = *buf.getStart<Data>(); }
};
...
class Item : public Row {
  public:
     ...
     * Constructor only with primary key.
     */
    explicit Item(uint32_t I_ID, uint32_t W_ID) : type(8), I_ID(I_ID), data(), W_ID(W_ID) {}

    virtual uint64_t tid() { return tableId[W_ID]; }
    virtual const void* pKey() { return &type; }
    virtual uint16_t pKeyLength() { return 1 + sizeof(I_ID); }
    virtual const void* value() { return &data; }
    virtual uint32_t valueLength() { return sizeof(data); }
    virtual void parseBuffer(Buffer& buf) { data = *buf.getStart<Data>(); }
};
...
class Stock : public Row {
  public:
    ...
    explicit Stock(uint32_t S_I_ID, uint32_t S_W_ID)
        : type(9), S_I_ID(S_I_ID), S_W_ID(S_W_ID), data() {}

    virtual uint64_t tid() { return tableId[S_W_ID]; }
    virtual const void* pKey() { return &type; }
    virtual uint16_t pKeyLength() { return 1+ sizeof(S_I_ID) + sizeof(S_W_ID); }
    virtual const void* value() { return &data; }
    virtual uint32_t valueLength() { return sizeof(data); }
    virtual void parseBuffer(Buffer& buf) { data = *buf.getStart<Data>(); }
};
...
class Driver {
  public:
    ...
    // returns latency.
    double txNewOrder(uint32_t W_ID, bool* outcome);
    double txPayment(uint32_t W_ID, bool* outcome);
    double txOrderStatus(uint32_t W_ID, bool* outcome);
    double txDelivery(uint32_t W_ID, bool* outcome);
