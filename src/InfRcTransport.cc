...
int InfRcTransport::MAX_TRANSPORTS = 0;
uint32_t InfRcTransport::SHARED_RX_QUEUE_DEPTH = InfRcTransport::MAX_SHARED_RX_QUEUE_DEPTH;
uint32_t InfRcTransport::TX_QUEUE_DEPTH = InfRcTransport::MAX_TX_QUEUE_DEPTH;
    ...
    , testingDontReallySend(false)
{
    ...
    static uint32_t maxRpcSize = Transport::MAX_RPC_LEN;
    if (!initConfig) {
        initConfig = true;
        ...
        if (configFile.is_open()) {
            ...
            if (params->hasOption("numRC")) {
                MAX_TRANSPORTS = params->getOption<int>("numRC");
            }
            if (NULL == sl) {
            ...
            if (params->hasOption("maxRpcSize")) {
                maxRpcSize = params->getOption<uint32_t>("maxRpcSize");
            }
        }
        LOG(NOTICE, "MAX_TRANSPORTS = %d, SHARED_RX_QUEUE_LENGTH = %u, "
                "TX_QUEUE_DEPTH = %u, maxRpcSize = %u", MAX_TRANSPORTS,
                SHARED_RX_QUEUE_DEPTH, TX_QUEUE_DEPTH, maxRpcSize);
    }
    ...
    uint32_t bufferSize = (maxRpcSize + 4095) & ~0xfff;
...
bool
InfRcTransport::hasQueuedRequests()
{
    return !clientSendQueue.empty();
}
...
#define LOG_TIME_SYNC(prefix, length, nonce) \
    if (length == sizeof(WireFormat::ReadTsc::Request)) { \
        timeTrace(prefix" ALL_DATA, clientId %u, sequence %u", \
                nonce >> 32, nonce); \
    }
    ...
    if (srq == clientSrq) {
        ...
        if (!clientSendQueue.empty()) {
            ...
            if (waitCycles > Cycles::fromMicroseconds(5)) {
                ...
                // buffers. Maybe consider increasing SHARED_RX_QUEUE_DEPTH?
...
InfRcTransport::getTransmitBuffer()
{
    ...
    while (freeTxBuffers.empty()) {
        ...
        if (freeTxBuffers.empty()) {
            ...
            // FIXME: increasing MAX_TX_QUEUE_DEPTH to a really large number
...
#define LARGE_COPIED_BYTES 10000
    ...
    if (expect_true(!testingDontReallySend)) {
        ...
        if (response) {
            LOG_TIME_SYNC("server sending", message->size(), nonce);
        } else {
            LOG_TIME_SYNC("client sending", message->size(), nonce);
        }
...
InfRcTransport::ClientRpc::ClientRpc(InfRcTransport* transport,
    ...
    , response(response)
    , notifier(notifier)
    , nonce((transport->clientId << 32) | (nonce >> 32))
//    , nonce(nonce)
    , waitStart(0)
    , state(PENDING)
    , queueEntries()
    ...
    if (!t->outstandingRpcs.empty()) {
        ...
        for (int i = 0; i < numResponses; i++) {
            ...
            foreach (ClientRpc& rpc, t->outstandingRpcs) {
                ...
                LOG_TIME_SYNC("client received", rpc.response->size(),
                        rpc.nonce);
    ...
    if (t->serverSetupSocket >= 0) {
        ...
        for (int i = 0; i < numRequests; i++) {
            ...
            bool returnBufDesc =
                    ...
                    ((numFreeBufDesc < d*2) && (len < 1000)) ||
                    ((numFreeBufDesc < d));
            if (returnBufDesc) {
                r->requestPayload.appendCopy(bd->buffer + sizeof(header), len);
                t->postSrqReceiveAndKickTransmit(t->serverSrq, bd);
                // FIXME: HOW TO DETERMINE THIS THRESHOLD
                if (len > LARGE_COPIED_BYTES) {
            ...
            timeTrace("server received request, client addr %u, nonce %u, "
                    ...
                    header.nonce, len);
            LOG_TIME_SYNC("server received", len, header.nonce);
